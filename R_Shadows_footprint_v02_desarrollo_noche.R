...
stylefile <- "Z:/De sastre/CAC/github/R_Shadows/styles/footprint_azul.qml"
stylefilenight <- "Z:/De sastre/CAC/github/R_Shadows/styles/footprint_gris.qml"
...
# Calcular sunrise y sunset para el dia y lugar
loc <- location_geo@coords
sunrise <- sunriset(loc, t1,direction="sunrise", POSIXct.out=TRUE)$time
sunset <- sunriset(loc, t1,direction="sunset", POSIXct.out=TRUE)$time

#CALCULO DE SOMBREADO
#guardar archivo de visualizacion para qgis y crea lista de nombres de archivos
shplist <- list()
while (t1<=t2) {
        # si la hora de t1 está entre sunrise y sunset --> calcular la footprint
        if (t1<=sunrise & t1<= sunset$time){
                ...
                # crear un shp de extension x y lo guarda con style que imita la noche
